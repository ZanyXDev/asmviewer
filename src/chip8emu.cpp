...
void Chip8Emu::executeNextOpcode()
{
    ...
    case 0x3: // 3xkk SE Vx, kk Пропустить следующую инструкцию, если регистр Vx = kk
        asmTextString.append(QString("SE V0x%1, 0x%2; Skip next instruction if V0x%1 = 0x%2").arg( X, 0, 16 ).arg( KK,0,16 ));
        break;
    case 0x4: // 4xkk SNE Vx, kk Пропустить следующую инструкцию, если регистр Vx != kk
        asmTextString.append(QString("SNE V0x%1, 0x%2; Skip next instruction if V0x%1 != 0x%2").arg( X, 0, 16 ).arg( KK, 0, 16 ));
        break;
    case 0x5: // 5xy0 SE Vx, Vy Пропустить следующую инструкцию, если Vx = Vy
        if ( LO == 0x0){
            asmTextString.append(QString("SE V0x%1, V0x%2; Skip next instruction if V0x%1 = V0x%2").arg( X,0,16 ).arg( Y,0,16 ));
        }else {
    ...
    case 0x6: // 6xkk LD Vx, kk Загрузить в регистр Vx число kk, т.е. Vx = kk
        asmTextString.append(QString("LD V0x%1, 0x%2; Load 0x%2 to register V0x%1 ").arg( X, 0, 16 ).arg( KK, 0, 16 ));
        break;
    case 0x7: // 7xkk ADD Vx, kk Установить Vx = Vx + kk
        asmTextString.append(QString("ADD V0x%1, 0x%2; V0x%1 = V0x%1 + 0x%2").arg( X,0,16 ).arg( KK, 0, 16 ));
        break;
    case 0x8:
        switch ( LO ){
    ...
    case 0xD: // Dxyn DRW Vx, Vy, n Нарисовать на экране спрайт. Эта инструкция считывает n байт по адресу
        ...
        asmTextString.append(QString("DRW V0x%1, 0x%2,0x%3; Draw sprite (0x%3 bytes) in the pos( 0x%1, 0x%2 ) ").arg( X,0,16 ).arg( Y,0,16 ).arg( LO,0,16 ));
        break;
    case 0xE:
    ...
    default:
        qDebug() << "opCode: " << opCode << " invalid.";
        asmTextString.append(QString("Don't decode PC=0x%1, 0x%2").arg(PC, 0,16).arg( opCode,0,16 ));
        break;
    }

    if ( m_enableListing && !asmTextString.isEmpty()){
        emit showDecodeOpCode (asmTextString);
        progText.append( qMakePair( PC, asmTextString ));
    }
